		...
		while (true) 
		{
		  ...
		  { // Send synchronously
			try 
			{
				...
				System.out.println("Mensaje enviado:. ( " + messageNo + ", " + messageStr + " )");
...
class DemoCallBack implements Callback {
	...
	public void onCompletion(RecordMetadata metadata, Exception exception) {
		...
		if (metadata != null) {
			System.out.println(
					"Mensaje enviado: (" + key + ", " + message + ") a la particion(" + metadata.partition() +
					"), " +
					"offset(" + metadata.offset() + ") en " + elapsedTime + " ms");
		} else {
